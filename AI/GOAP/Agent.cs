	...
	public abstract class Agent
	{
		...
		public bool plan( bool debugPlan = false )
		{
			...
			if( nodes != null && nodes.Count > 0 )
			{
				...
				Debug.Log( string.Format("plan cost = {0}\n", nodes[nodes.Count - 1].costSoFar) );
				Debug.Log( string.Format("{0}\t{1}", "start".PadRight( 15 ), getWorldState().describe( _planner )) );
				for( var i = 0; i < nodes.Count; i++ )
				{
					Debug.Log( string.Format("{0}: {1}\t{2}", i, nodes[i].action.GetType().Name.PadRight( 15 ), nodes[i].worldState.describe( _planner )) );
